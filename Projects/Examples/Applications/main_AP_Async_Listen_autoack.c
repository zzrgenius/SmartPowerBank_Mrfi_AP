...
int mrfi_loop(void)
{
	static uint32_t MrfiTicks = 0;
	connInfo_t *tConnInfo;	 
	static T_MSG_RESP tRespMsg; 
	u16 wlen;
	static 	uint32_t dwGprsStatus_m = 0;
	if((uint32_t)(SysRunTicks -  MrfiTicks) > 10)
	{
		MrfiTicks = SysRunTicks;
		...
		if (sPeerFrameSem)
		{
			...
			for (i = 0; i < sNumCurrentPeers; ++i)
			{
				if (SMPL_SUCCESS == SMPL_Receive(sLID[i], msg, &len))
				{
					...
					tConnInfo = nwk_getConnInfo(sLID[i]);
					if(tConnInfo != NULL)
					{						
...
//						tRespMsg.Data[1] = 0;//tGetPar.VerSystem;
						...
						if((len+5) > RESP_PAYLOAD_SIZE)
							len = RESP_PAYLOAD_SIZE-5;						
						memcpy(&tRespMsg.Data[NET_ADDR_SIZE+1],msg,len);
						MsgPackage(&tRespMsg,MSG_TYPE_RESP_GPRS);
//						Gprs_GetConnectStatus(&dwGprsStatus_m);
//						wlen =  tRespMsg.wLen[0] +(tRespMsg.wLen[1]<<8);
//						if(dwGprsStatus_m == GPRS_TCP_OK)
//						{						 
...
//		}
		if (BSP_BUTTON1())
		{
			...
			if(i > NUM_CONNECTIONS)
				i = 0;
			if (SMPL_SUCCESS == SMPL_SendOpt((linkID_t)i,msg,len,SMPL_TXOPTION_ACKREQ))
			{
					toggleLED(1);
			}		
