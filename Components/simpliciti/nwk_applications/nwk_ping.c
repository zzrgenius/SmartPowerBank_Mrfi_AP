...
smplStatus_t nwk_ping(linkID_t lid)
{
	...
	do
	{
		...
		else
		{
			...
			if ((numChan = nwk_scanForChannels(channels)) == 0)
			{
...
static void smpl_send_ping_reply(mrfiPacket_t *frame)
{
	 ...
	 */
	if ((pOutFrame = nwk_buildFrame(SMPL_PORT_PING, MRFI_P_PAYLOAD(frame)+F_APP_PAYLOAD_OS, MRFI_GET_PAYLOAD_LEN(frame)-F_APP_PAYLOAD_OS, MAX_HOPS)) != NULL)
	{
