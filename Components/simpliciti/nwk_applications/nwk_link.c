...
void nwk_linkInit(void)
{
	...
	/* set a non-zero TID. */
	while ((sTid = MRFI_RandomByte()) == NULL)  ;
...
static void smpl_send_unlink_reply(mrfiPacket_t *frame)
{
  ...
  if ((pCInfo = nwk_findPeer((addr_t *)MRFI_P_SRC_ADDR(frame), *(MRFI_P_PAYLOAD(frame)+F_APP_PAYLOAD_OS+UL_RMT_PORT_OS))) != NULL)
  {
  ...
  if ((pOutFrame = nwk_buildFrame(SMPL_PORT_LINK, msg, sizeof(msg), MAX_HOPS)) != NULL)
  {
...
static uint8_t smpl_send_link_reply(mrfiPacket_t *frame)
{
	...
	if ((pCInfo = nwk_isLinkDuplicate(MRFI_P_SRC_ADDR(frame), remotePort)) != NULL)
	{
		/* resend reply */
	...
	#endif
		if ((pOutFrame = nwk_buildFrame(SMPL_PORT_LINK, msg, sizeof(msg), MAX_HOPS-(GET_FROM_FRAME(MRFI_P_PAYLOAD(frame),F_HOP_COUNT)))) != NULL)
		{
			/* destination address is the source adddress of the received frame. */
			memcpy(MRFI_P_DST_ADDR(&pOutFrame->mrfiPkt), MRFI_P_SRC_ADDR(frame), NET_ADDR_SIZE);
	#if defined(SMPL_SECURE)
